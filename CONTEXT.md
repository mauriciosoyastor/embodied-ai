...
- **Golden Path Fusión**: pipeline único y obligatorio `Issue → triage(needs-triage→ready-for-agent) → wayfinder? → to-tickets(tracer-bullet) → gitnexus-plan(impact+pdg) → gitnexus-work(+harness verify) → code-review(two-axes) → merge humano`. Single-writer `trajectory.jsonl`, `sandbox-edit` default, `MUST impact before edit / detect_changes before commit`. Define el orden canónico de las tres metodologías (1) Matt Pocock (2) GitNexus (3) repo `AGENTS.md`+Harness; ver `docs/adr/0007-fusion-three-methodologies.md`.
