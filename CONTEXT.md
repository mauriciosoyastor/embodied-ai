...
- **PromptList Estática**: `YOLO_WORLD_PROMPTLIST_STATIC: list[str]` 20 clases atómicas COCO en inglés CLIP (`person, chair, couch, dining table, bed, toilet, tv, laptop, keyboard, mouse, cell phone, remote, bottle, cup, wine glass, bowl, book, backpack, handbag, potted plant` `config.py:34`) para loop visual continuo `slow 2Hz` en `YoloWorldDetector` `yolo_world.py:32`; `list[str]` con sanitización `cleaned[:8]` `yolo_world.py:51`; caché `txt_feats 20x512` congelada al boot (`offline Text Feature Caching`) sin re-encode por frame (`+5ms` ahorrado `r2:138`); `box_thr 0.35 text_thr 0.25` `r2:128` sin `""` background.
- **PromptList Dinámica**: `YoloWorldDetector.set_classes(prompts: list[str])` inyecta máx 8 prompts libres por voz vía `extract_prompts_from_transcript` `yolo_world.py:95` (triggers `mirá/buscá/dónde está/qué color` + regex `[^.?]+` split `y/,/con` `r2:284`) con debounce 500ms cooldown 2s; solo si `YOLO_WORLD_DYNAMIC_BY_VOZ=True` `config.py:33` (default `False`); `asyncio Task` secundario parsea voz+mapeo `en<->es-AR` sin bloquear `VideoCapture`.
