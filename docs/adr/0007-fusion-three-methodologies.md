...
- **Downstream**: tickets #123-#128 desbloqueados; `harness/context_injector.py` y `impact_ratio` quedan para prototipos; `pre-commit` `ruff+mypy+pytest` es gate P0 (requiere `pre-commit` en `dev` deps pyproject.toml), GitNexus gate es P1 en Harness.
